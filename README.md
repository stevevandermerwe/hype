...
**Change one slide.** Select a slide and click **AI** above the editor (or press **Ctrl+J**), then say what you want. Choose **Text** to rewrite the slide, **Diagram** to rewrite it and draw an SVG picture beside it, or **Image** to add a generated picture and leave the text alone. The change applies at once and **Ctrl+Z** undoes it. The model sees the slide and a numbered outline of the whole deck, so edits stay consistent with the talk; it never sees other slides in full. Pictures are saved in `images/` without replacing existing files, so save the presentation first. **Image** uses a separate image model (`google/gemini-2.5-flash-image` on OpenRouter by default) with the same API key; set its model and endpoint in the dialog. Use an OpenRouter-style chat endpoint, or an OpenAI-style `/v1/images/generations` URL. From a terminal: `hype revise talk.md 3 "make it punchier"`, with `--diagram` or `--image`.
